...
def get_weather_data(query_url: String, bools: PythonObject) -> PythonObject: #Performs API call with given url and returns a dictionary
    ...
    try: #IF the API call errors
        ...
        try:   
            ...
            for i in range(keys.__len__().__index__()): 
...
def start_weather_chat(inout time_list: SIMD, inout val_stuff: Int):
    Python.add_to_path(".")
...
def main():
    var time_list = SIMD[DType.float64,8]()
    ...
    for x in range(len(time_list)):
        print(time_list[x]/1000000, "seconds") #SIMD operation
